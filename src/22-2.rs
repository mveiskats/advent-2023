...
fn main() {
    ...
    let result: usize = (0..bricks.len()).map(|i| {
        let mut falling: Vec<bool> = vec![false; bricks.len()];
        falling[i] = true; // Disintegrate

        let mut other_falling = 0;
        let mut more_falling = 1;

        while more_falling > 0 {
            more_falling = 0;
            for j in 0..bricks.len() {
                if !falling[j] && supported_by[j].len() > 0 && supported_by[j].iter().all(|&supp| falling[supp]) {
                    falling[j] = true;
                    more_falling += 1;
                }
            }
            other_falling += more_falling;
        }
